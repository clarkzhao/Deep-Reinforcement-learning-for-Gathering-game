...
class ReplayBuffer(object):
    def __init__(self, size, frame_history_len=DQNSetting.N_HIST):
...
class ReplayBufferRU(object):
    def __init__(self, size, frame_history_len=DQNSetting.N_HIST):
        """This is a memory efficient implementation of the replay buffer for repeated update heuristic
    ...
    def _encode_sample(self, idxes):
        ...
        act_batch = self.action[idxes]
        rew_batch = self.reward[idxes]
        eff_lr_batch = self.eff_lr[idxes]
        next_obs_batch = np.concatenate([self._encode_observation(idx + 1)[np.newaxis, :] for idx in idxes], 0)
        done_mask = np.array([1.0 if self.done[idx] else 0.0 for idx in idxes], dtype=np.float32)

        return obs_batch, act_batch, rew_batch, next_obs_batch, done_mask, eff_lr_batch
    ...
    def store_frame(self, frame):
        ...
        if self.obs is None:
            ...
            self.reward   = np.empty([self.size],                     dtype=np.float32)
            self.done     = np.empty([self.size],                     dtype=np.bool)
            self.eff_lr = np.empty([self.size],                 dtype=np.float32)
    ...
    def store_effect(self, idx, action, reward, done, eff_lr):
        """Store effects of action taken after obeserving frame stored
        at index idx. The reason `store_frame` and `store_effect` is broken
        up into two functions is so that one can call `encode_recent_observation`
        in between.
